/**
 * Importa el export "Turnos detallados" de Alephoo (.xls que en realidad es
 * una tabla HTML) con turnos FUTUROS:
 ...
 *    a partir de los turnos reales del archivo
 ...
 *
 * Uso: pnpm --filter @workspace/scripts run importar-turnos-detallados <archivo.xls>
 */
...
function parsearArchivo(archivo: string): FilaTurno[] {
  // El export viene en UTF-16/latin1 con bytes NUL intercalados: leer como
  // latin1 y strippear \x00 conserva los acentos (leerlo como utf-8 los rompe).
  const html = fs.readFileSync(archivo, "latin1").replace(/\u0000/g, "");
...
async function main() {
  ...
  if (!archivo || !fs.existsSync(archivo)) {
    console.error("Uso: importar-turnos-detallados <archivo.xls>");
    process.exit(1);
  }

  console.log(`📄 Parseando ${path.basename(archivo)}...`);
  const filas = parsearArchivo(archivo);
  console.log(`✅ ${filas.length} turnos válidos parseados`);
  ...
  console.log(`🗂  ${grupos.size} turneras (sede+especialidad+médico) detectadas`);
  ...
  console.log(`🏥 Turneras: ${contadores.turnerasCreadas} creadas, ${contadores.turnerasActualizadas} acomodadas`);
  ...
  for (const [dni, f] of filaPorDni) {
    ...
    try {
      ...
      else console.error(`⚠️  Paciente DNI ${dni}: ${err instanceof Error ? err.message : err}`);
    }
  }
  console.log(`👥 Pacientes: ${contadores.pacientesCreados} creados, ${contadores.pacientesActualizados} actualizados (${contadores.pacientesReactivados} reactivados)`);
  ...
  for (let i = 0; i < aInsertar.length; i += 500) {
    ...
    try {
      ...
      // Conflicto en el lote (slot único): reintentar fila por fila
      for (const t of lote) {
        try {
          ...
          try {
            ...
            console.error(`⚠️  Turno omitido (${t.fecha} ${t.horaInicio} turnera ${t.turneraId}): ${err2 instanceof Error ? err2.message.slice(0, 120) : err2}`);
          }
        }
      }
    }
    if ((i / 500) % 5 === 0) console.log(`   ... ${Math.min(i + 500, aInsertar.length)}/${aInsertar.length} turnos`);
  }
  ...
  await db.insert(importacionesAdminTable).values({
    ...
    ejecutadoPor: "script importar-turnos-detallados",
    detalle: contadores,
  });
